package M3MTA::Server::SMTP::RFC4954::CRAM_MD5;
...
sub initial_response {
	...
	if(!$args) {
        ...
        $session->respond($M3MTA::Server::SMTP::ReplyCodes{SERVER_CHALLENGE}, $md5_challenge);
		return;
	}
...
sub data {
    ...
    if($pass ne $b64) {
        ...
        $session->respond($M3MTA::Server::SMTP::ReplyCodes{AUTHENTICATION_FAILED}, "CRAM-MD5 authentication failed");
        $session->user(undef);
    } else {
