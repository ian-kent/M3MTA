...
M3MTA::Server::SMTP::RFC2487- STARTTLS extension
...
sub register {
	...
	if(!$smtp->has_rfc('RFC5321')) {
        die "M3MTA::Server::SMTP::RFC2487 requires RFC5321";
    }
        ...
        if($smtp->config->{extensions}->{starttls}->{require_always}) {
            $smtp->register_hook('command', sub {
                ...
                if($cmd !~ /^(STARTTLS|HELO|EHLO|QUIT|NOOP)$/ && !$session->{tls_enabled}) {
                    ...
                    $result->{response} = [$M3MTA::Server::SMTP::ReplyCodes{STARTTLS_REQUIRED}, "must issue a STARTTLS command first"];
                    return 1;
                }
...
sub starttls {
	...
	$session->respond($M3MTA::Server::SMTP::ReplyCodes{SERVICE_READY}, "Ready to start TLS");
    ...
    if(!$session->{tls_enabled}) {
        ...
        if(my ($recipient) = $data =~ /^To:\s*<(.+)>$/i) {
            ...
            if(!$mailbox) {
                ...
                $session->respond($M3MTA::Server::SMTP::ReplyCodes{STARTTLS_REQUIRED}, "must issue a STARTTLS command first");
                return;
            }
        }
    }
